...
export class CotizacionIndiceService {
  ...
  async ultimaFechaDeCotizacionEnMiDB(codigoIndice: string): Promise<IFecha> {
    try {
      const indice = await this.indiceRepository.findOne({ where: { codigoIndice } });
      if (!indice) {
        console.log(`No se encontró una empresa con codEmpresa: ${codigoIndice}`);
        return null;
      }
      const lastCotizacion: CotizacionIndice[] = await this.cotizacionIndiceRepository.find(
        {
          where: { codigoIndice: Equal(codigoIndice) },
          order: { id: "DESC" },
          take: 1,
        })
      ...
      if (!dateCotizacion || !dateCotizacion.fecha) {
        ...
        console.log(`NO SE ENCONTRO FECHA PARA ${codigoIndice}, ASIGNADA FECHA:'2024-01-01' HORA:'00:00'`)
        return fecha;
      } else {
        const fecha: IFecha = DateMomentsUtils.transformarFechaAGMT(dateCotizacion.fecha, dateCotizacion.hora);
        return fecha;
      }
  ...
  public async getCotizacionesDeGempresaConCodEmpresaYFechasEnGMT(codigoIndice: string, stringUltimaFechaEnMiDB: string, stringUltimaFechaDeGempresa: string): Promise<CotizacionIndice[]> {
    ...
    this.logger.log(`CIS - URL formada para la API externa: ${url}`);
    ...
    const cotizacionesFaltantes = await Promise.all(respuesta.data.map(async (cotizacion) => {
      ...
      if (horarioDeBolsaUTC.includes(fechaUTC.hora)) {
        ...
        this.logger.log(`CIS - Guardando cotización: ${JSON.stringify(nuevaCotizacionIndice)}`);
  ...
  public async guardarCotizacionEnDB(cotizacionIndice: CotizacionIndice): Promise<CotizacionIndice> {
    try {
      this.logger.log("CIS - Intentando guardar la cotización:", JSON.stringify(cotizacionIndice));
      ...
      if (!hayCotizacion) {
        this.logger.log("CIS - No se encontró cotización existente, guardando nueva cotización");
